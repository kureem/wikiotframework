...
public class EXTable extends EXContainer implements Table, Event
{
	...
	public void onReady(JQuery query){
		try{
			JSMap options = new JSMap(); 
			options.put("total", getPages());
			
			JQuery serverrequest =query.clone().makeServerRequest(new JSMap().put("num", new JSVar("num")),this);
			JSFunction function = new JSFunction(serverrequest,new JSVar("event"), new JSVar("num"));
