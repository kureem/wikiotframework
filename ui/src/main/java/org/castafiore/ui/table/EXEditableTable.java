...
import org.castafiore.ui.dropdown.EXDropdown;
...
public class EXEditableTable extends EXTable implements Event {
		...
		for (int row = 0; row < rowsToShow; row++) {
			...
			for (int col = 0; col < tableModel_.getColumnCount(); col++) {
				...
				if (tr.getChildren().size() <= col) {
					td = new EXContainer("td", "td").setAttribute("c", col + "").setAttribute("r", row + "")
							.setAttribute("m", "r");
	...
	public void startEdit(int row, int col, int page, Container container) {
		...
		FormInput sft = ((EditableCellRenderer) this.celleRendere_).getInputAt(row, col, page,
		...
		if (sft instanceof EXDropdown) {
			sft.getChildByIndex(0).setStyle("margin", "0").setStyle("height", "27px").addEvent(this, BLUR)
					.setAttribute("r", row + "").setAttribute("c", col + "").setAttribute("m", "e");
			// ((EXInput)sft.getChildByIndex(0)).requestFocus();
			container.addChild(sft);
		} else {

			container.addChild(sft.addEvent(this, BLUR).addEvent(NAVIGATE, KEY_DOWN).setAttribute("r", row + "")
					.setStyle("width", "100%").setAttribute("c", col + "").setAttribute("m", "e")
					.setStyle("margin", "0").setStyle("height", "27px"));
			if (sft instanceof AbstractFormComponent) {
				((AbstractFormComponent) sft).requestFocus();
			}
		}
	...
	public void cancelEdit(int row, int col, int page, Container container) {
		...
		FormInput<?> stf = container.getAncestorOfType(FormInput.class);
	...
	public static Event NAVIGATE = new Event() {
		...
		@Override
		public void ClientAction(JQuery container) {
			JSVar key = new JSVar("(event.keyCode || event.which)");
			container.IF(key.equal(new JSVar("9")), container.clone().server(this), container.clone());
