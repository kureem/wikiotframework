...
public class AdvancedDroppableEvent  implements Event{
	...
	public void ClientAction(JQuery application) {
		...
		JSMap options = droppable.getDroppableOptions();
		if(options == null)
		{
			options = new JSMap();
		}
		...
		for(String m : methods)
		{
			...
			JQuery clone = application.clone();
	...
	public void Success(JQuery component,
	...
	public boolean ServerAction(Container component,
			Map<String, String> requestParameters) throws UIException {
		/*String method = requestParameters.get("action");
		
		String sourceId = requestParameters.get("source");
		
		Container source = ComponentUtil.getContainerById(component.getRoot(), sourceId);
		Droppable droppable = (Droppable)component;
		if(method.equalsIgnoreCase("drop"))
		{
			droppable.OnDrop(source);
		}
		
		else if(method.equalsIgnoreCase("deactivate"))
		{
			droppable.onDeactivate(source);
		}
		
		else if(method.equalsIgnoreCase("over"))
		{
			droppable.onOver(source);
		}
		else if(method.equalsIgnoreCase("out"))
		{
			droppable.onOut(source);
		}*/
		return false;
		//return true;
