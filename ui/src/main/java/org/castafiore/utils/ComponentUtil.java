...
public class ComponentUtil {
	...
	public static void fastExtractComponents(Container container, List<FormInput> result, List<Container> ccs,
			String componentId) {
		compcount++;
		if (container == null) {
			return;
		...
		if (container instanceof FormInput) {
			result.add((FormInput) container);
		}
	...
	@SuppressWarnings("rawtypes")
	public static void getAllStatefullDescendents(Container container, List<FormInput> result) {
		if (container == null) {
			return;
		}
		if (container instanceof FormInput) {
	...
	public static void loadApplication(HttpServletRequest request, HttpServletResponse response,
			ApplicationRegistry registry) throws ServletException {
		...
		if (applicationId != null) {
			...
			if (application == null && applicationId != null) {
				...
				// logger.debug("application :" + applicationId + " not found in
				// session. Loading fresh application from registry");
				application = registry.getApplication(request, response);
				if (application == null) {
					// logger.error("there is no bean with name " +
					// applicationId + " configured in application registry");
				...
				if (!application.getName().equals(applicationId)) {
					// logger.error("The application Id configured in
					// application-registry should be the same as the
					// application name");
					throw new ServletException(
