...
public class wavIO {
   ...
   * two fields not included in this count: ChunkID and ChunkSize. 8 4 Format
   * Contains the letters "WAVE" (0x57415645 big-endian form).
   * 
   * The "WAVE" format consists of two subchunks: "fmt " and "data": The "fmt "
   * subchunk describes the sound data's format: 12 4 Subchunk1ID Contains the
   * letters "fmt " (0x666d7420 big-endian form). 16 4 Subchunk1Size 16 for PCM.
   * This is the size of the rest of the Subchunk which follows this number. 20
   * 2 AudioFormat PCM = 1 (i.e. Linear quantization) Values other than 1
   * indicate some form of compression. 22 2 NumChannels Mono = 1, Stereo = 2,
   * etc. 24 4 SampleRate 8000, 44100, etc. 28 4 ByteRate == SampleRate *
   ...
   * I wonder what happens when this number isn't an integer? 34 2 BitsPerSample
   * 8 bits = 8, 16 bits = 16, etc.
   * 
   * The "data" subchunk contains the size of the data and the actual sound: 36
   * 4 Subchunk2ID Contains the letters "data" (0x64617461 big-endian form). 40
   * 4 Subchunk2Size == NumSamples * NumChannels * BitsPerSample/8 This is the
   * number of bytes in the data. You can also think of this as the size of the
   * read of the subchunk following this number. 44 * Data The actual sound
   * data.
   * 
   * 
   * NOTE TO READERS:
   * 
   * The thing that makes reading wav files tricky is that java has no unsigned
   * types. This means that the binary data can't just be read and cast
   * appropriately. Also, we have to use larger types than are normally
   * necessary.
   * 
   * In many languages including java, an integer is represented by 4 bytes. The
   * issue here is that in most languages, integers can be signed or unsigned,
   * and in wav files the integers are unsigned. So, to make sure that we can
   * store the proper values, we have to use longs to hold integers, and
   * integers to hold shorts.
   * 
   * Then, we have to convert back when we want to save our wav data.
   * 
   * It's complicated, but ultimately, it just results in a few extra functions
   * at the bottom of this file. Once you understand the issue, there is no
   * reason to pay any more attention to it.
   * 
   * 
   * ALSO:
   * 
...
public boolean read() {
  ...
  public String getSummary() {
    ...
    String summary = "<html>Format: " + myFormat + newline + "Channels: "
        + myChannels + newline + "SampleRate: " + mySampleRate + newline
        + "ByteRate: " + myByteRate + newline + "BlockAlign: " + myBlockAlign
        + newline + "BitsPerSample: " + myBitsPerSample + newline
        + "DataSize: " + myDataSize + "</html>";
    return summary;
  }
